  ...
  return (
    <Dialog open onOpenChange={(open) => { if (!open && !isPending) onClose(); }}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="text-sm font-semibold">Disconnect repository</DialogTitle>
          <DialogDescription className="text-xs leading-relaxed">
            <span className="font-medium text-gray-700 dark:text-gray-300">{repoName}</span>{" "}
              ...
              >
                <div className="flex items-start gap-3">
                  <div
                    ...
                    style={{ borderColor: isActive ? opt.radioColor : undefined }}
                  >
                    {isActive && (
                      <span
